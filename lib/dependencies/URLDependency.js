...
/** @typedef {import("../serialization/ObjectMiddleware").ObjectDeserializerContext<[Range, boolean, EntryOptions | undefined, UsedByExports | undefined]>} ObjectDeserializerContext */
/** @typedef {import("../serialization/ObjectMiddleware").ObjectSerializerContext<[Range, boolean, EntryOptions | undefined, UsedByExports | undefined]>} ObjectSerializerContext */
...
URLDependency.Template = class URLDependencyTemplate extends (
	...
	apply(dependency, source, templateContext) {
		...
		// Entry-block mode: the dependency was promoted to an
		// AsyncDependenciesBlock with entryOptions.
		...
		if (
			...
			const getChunkFilename =
				/** @type {string} */
				(
					URLEntryPlugin.getCompilationHooks(compilation).getChunkFilename.call(
						chunk,
						dep
			...
			source.replace(
				dep.outerRange[0],
				dep.outerRange[1] - 1,
				`/* entry url */ new URL(${RuntimeGlobals.publicPath} + ${getChunkFilename}(${JSON.stringify(chunk.id)}), ${RuntimeGlobals.baseURI})`
			);
			return;
		}
